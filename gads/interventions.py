...
(`mark_write_attempt()` volají `_run_mutation` a všechna místa, která mutují mimo
něj). Když příkaz skončí dřív (validace, `_die`, nenalezený objekt), ticket zůstává
`claimed` a dá se použít znovu.
...
# Stav jednoho běhu CLI: posbírané resource names + zda se o mutaci vůbec pokusilo.
_collected: list[str] = []
...
def mark_write_attempt() -> None:
    """Volat těsně PŘED odesláním skutečné mutace do Google Ads."""
    global _write_attempted
...
def record_result(response) -> None:
    """Posbírej resource names z odpovědi mutace (results, atomic mutate,
    long-running operation `name`). Best effort, nikdy nevyhodí."""
    mark_write_attempt()
    try:
        ...
        for r in getattr(response, "mutate_operation_responses", None) or []:
            ...
            for _descriptor, value in pb.ListFields():
                ...
                if rn:
                    _collected.append(rn)
        lro = getattr(response, "name", None)
        if isinstance(lro, str) and lro.startswith("operations/") and lro not in _collected:
            _collected.append(lro)
...
def _journal_line(ctx: TicketContext, exit_ok: bool) -> str:
...
def _postflight(ctx: TicketContext | None, exit_ok: bool) -> None:
    ...
    if ctx.mode == "lite":
        ...
        with ctx.journal_path.open("a", encoding="utf-8") as fh:
            fh.write(_journal_line(ctx, exit_ok))
        return
